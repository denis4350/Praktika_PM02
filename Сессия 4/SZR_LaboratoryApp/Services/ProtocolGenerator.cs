...
namespace SZR_LaboratoryApp.Services
{
    ...
    {
        public static void GenerateTestProtocol(LabTest test, object batch, string batchType, string outputPath)
        {
            using (var document = new PdfDocument())
            {
                ...
                using (var gfx = XGraphics.FromPdfPage(page))
                {
                    ...
                    double yPos = 40;
                    double pageHeight = page.Height.Point; // Используем Point вместо implicit conversion
                    ...
                    gfx.DrawString("1. ИНФОРМАЦИЯ ОБ ИСПЫТАНИИ", headerFont, XBrushes.Black,
                        new XRect(40, yPos, pageWidth - 80, 20), XStringFormats.TopLeft);
                    yPos += 25;
                    ...
                    gfx.DrawString("2. ИНФОРМАЦИЯ ОБ ОБЪЕКТЕ КОНТРОЛЯ", headerFont, XBrushes.Black,
                        new XRect(40, yPos, pageWidth - 80, 20), XStringFormats.TopLeft);
                    yPos += 25;

                    if (batchType == "RawMaterial" && batch is RawMaterialBatch rawBatch)
                    {
                        DrawInfoRow(gfx, normalFont, ref yPos, "Тип объекта:", "Партия сырья");
                    ...
                    else if (batchType == "Product" && batch is ProductBatch productBatch)
                    {
                        DrawInfoRow(gfx, normalFont, ref yPos, "Тип объекта:", "Партия готовой продукции");
                    ...
                    DrawResultsTable(gfx, normalFont, smallFont, ref yPos, test, pageWidth, pageHeight);
                    ...
                    // ПОДПИСИ
                    double signatureY = pageHeight - 60;
        ...
        private static void DrawResultsTable(XGraphics gfx, XFont font, XFont smallFont, ref double yPos, LabTest test, double pageWidth, double pageHeight)
        {
            ...
            double startX = 40;
            double[] colWidths = { 30, 120, 80, 80, 80, 80, 60 };
            ...
            for (int i = 0; i < headers.Length; i++)
            {
                ...
                gfx.DrawString(headers[i], smallFont, XBrushes.Black,
            ...
            foreach (var param in test.parameters)
            {
                ...
                gfx.DrawString(rowNum.ToString(), smallFont, XBrushes.Black,
                ...
                gfx.DrawString(param.ParameterName ?? "", smallFont, XBrushes.Black,
                ...
                gfx.DrawString(param.NormMin?.ToString() ?? "—", smallFont, XBrushes.Black,
                ...
                gfx.DrawString(param.NormMax?.ToString() ?? "—", smallFont, XBrushes.Black,
                ...
                var actualColor = param.IsPassed == true ? XBrushes.Green : (param.IsPassed == false ? XBrushes.Red : XBrushes.Black);
                gfx.DrawString(param.ActualValue?.ToString("0.00") ?? "—", smallFont, actualColor,
                ...
                gfx.DrawString(param.Unit ?? "", smallFont, XBrushes.Black,
                ...
                gfx.DrawString(result, smallFont, resultColor,
        ...
        private static string GetConclusion(LabTest test)
        {
            ...
            if (allPassed)
                return "По результатам испытаний партия СООТВЕТСТВУЕТ установленным требованиям. Рекомендуется к использованию.";
            else if (anyFailed)
                return "По результатам испытаний партия НЕ СООТВЕТСТВУЕТ установленным требованиям. Рекомендуется к блокировке.";
            else
                return "Результаты испытаний требуют дополнительной проверки. Рекомендуется провести повторный анализ.";
        }
    }
}